<div class="relative min-h-screen transition-colors duration-500">
    <div class="w-full max-w-2xl mx-auto px-4 py-6 sm:py-10">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.04)] p-3 mb-8 border border-slate-100 dark:border-slate-700 focus-within:border-indigo-200 dark:focus-within:border-indigo-500 transition-all">
            <form wire:submit.prevent="addTask" class="flex flex-col sm:flex-row gap-2">
                ...
                <div class="flex items-center gap-2 p-1 sm:p-0">
                    ...
                    <button type="submit" wire:loading.attr="disabled" class="bg-indigo-600 hover:bg-indigo-700 text-white p-4 rounded-2xl shadow-xl transition-all active:scale-95 group flex items-center justify-center min-w-[56px]">
                        <svg wire:loading.remove xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 group-hover:rotate-90 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 4v16m8-8H4" /></svg>
        ...
        <div class="flex gap-2 mb-6 bg-slate-100 dark:bg-slate-900/50 p-1.5 rounded-2xl w-fit">
            <button wire:click="$set('filter', 'all')" class="px-5 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all {{ $filter === 'all' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300' }}">Toutes</button>
            <button wire:click="$set('filter', 'todo')" class="px-5 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all {{ $filter === 'todo' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300' }}">À faire</button>
            <button wire:click="$set('filter', 'completed')" class="px-5 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all {{ $filter === 'completed' ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300' }}">Faites</button>
        </div>
        ...
        <div class="grid gap-4">
            ...
            @empty
                <div class="text-center py-20 bg-white dark:bg-slate-800 rounded-[3rem] border border-dashed border-slate-200 dark:border-slate-700 transition-colors">
                    <div class="w-20 h-20 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-500 dark:text-indigo-400 rounded-full flex items-center justify-center mx-auto mb-6 text-3xl">✨</div>
                    <h3 class="text-xl font-black text-slate-800 dark:text-white tracking-tight">C'est le calme plat...</h3>
                    <p class="text-slate-400 dark:text-slate-500 font-medium max-w-xs mx-auto mt-2">Rien de prévu pour ce jour.</p>
                </div>
            @endforelse
        </div>
    </div>
